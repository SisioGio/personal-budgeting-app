      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
        ...
        <div className="rounded-xl bg-gradient-to-br from-blue-900/40 to-blue-800/20 p-4 border border-blue-700/50">
          <div className="flex items-center justify-between mb-2">
            <div className="text-blue-400 text-xs font-semibold uppercase tracking-wide">Current Balance</div>
            <span className="text-2xl">💰</span>
          </div>
          <div className="text-2xl font-bold text-white mb-1">${kpis.currentBalance.toFixed(2)}</div>
          <div className="text-[10px] text-gray-400">Total balance including actuals</div>
        </div>

        {/* Expenses vs Budget */}
        <div className="rounded-xl bg-gradient-to-br from-red-900/40 to-red-800/20 p-4 border border-red-700/50">
          <div className="flex items-center justify-between mb-2">
            <div className="text-red-400 text-xs font-semibold uppercase tracking-wide">Expenses</div>
            <span className="text-2xl">💸</span>
          </div>
          <div className="flex items-baseline gap-2 mb-1">
            <div className="text-2xl font-bold text-white">${kpis.expenseActual.toFixed(0)}</div>
            <div className="text-sm text-gray-400">/ ${kpis.expenseBudget.toFixed(0)}</div>
          </div>
          <div className="flex items-center gap-2">
            <div className={`text-xs font-semibold ${kpis.expensePercentage <= 100 ? 'text-green-400' : 'text-red-400'}`}>
        ...
        <div className="rounded-xl bg-gradient-to-br from-green-900/40 to-green-800/20 p-4 border border-green-700/50">
          <div className="flex items-center justify-between mb-2">
            <div className="text-green-400 text-xs font-semibold uppercase tracking-wide">Income</div>
            <span className="text-2xl">📈</span>
          </div>
          <div className="flex items-baseline gap-2 mb-1">
            <div className="text-2xl font-bold text-white">${kpis.incomeActual.toFixed(0)}</div>
            <div className="text-sm text-gray-400">/ ${kpis.incomeBudget.toFixed(0)}</div>
          </div>
          <div className={`text-xs font-semibold ${kpis.incomePercentage >= 100 ? 'text-green-400' : 'text-yellow-400'}`}>
            {kpis.incomePercentage.toFixed(0)}% achieved
          </div>
        </div>

        {/* Savings Rate */}
        <div className="rounded-xl bg-gradient-to-br from-purple-900/40 to-purple-800/20 p-4 border border-purple-700/50">
          <div className="flex items-center justify-between mb-2">
            <div className="text-purple-400 text-xs font-semibold uppercase tracking-wide">Net Income</div>
            <span className="text-2xl">💎</span>
          </div>
          <div className="text-2xl font-bold text-white mb-1">
            ${kpis.netIncome.toFixed(2)}
          </div>
          <div className={`text-xs font-semibold ${kpis.savingsRate >= 0 ? 'text-green-400' : 'text-red-400'}`}>
            {kpis.savingsRate >= 0 ? 'Saving' : 'Deficit'} {Math.abs(kpis.savingsRate).toFixed(0)}%
          </div>
        </div>

        {/* Budget Adherence Score */}
        <div className="rounded-xl bg-gradient-to-br from-yellow-900/40 to-yellow-800/20 p-4 border border-yellow-700/50">
          <div className="flex items-center justify-between mb-2">
            <div className="text-yellow-400 text-xs font-semibold uppercase tracking-wide">Budget Score</div>
            <span className="text-2xl">🎯</span>
          </div>
          <div className="text-2xl font-bold text-white mb-1">{kpis.budgetAdherence.toFixed(0)}/100</div>
